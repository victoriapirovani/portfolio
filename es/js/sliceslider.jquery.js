  ...
  $.fn.sliceSlider = function (options) {
    ...
    return this.each(function () {
      var $this = $(this),
        ...
        $rightWrapper = $this.find(
          '.slice-slider-right-column .slice-slider-wrapper'
        ),
        $rightSlide = $this.find(
          '.slice-slider-right-column .slice-slider-slide'
        ),
      ...
      $(
        '<div class="slice-slider-left-column"><div class="slice-slider-wrapper">' +
          $leftColumnContent +
          '</div></div>'
      ).insertBefore($rightColumn)
      if ($length > 1)
        $this.append(
          '<div class="pagination"><div class="point active"></div>' +
            '<div class="point"></div>'.times($length - 1) +
            '</div>'
        )

      var $leftColumn = $this.find('.slice-slider-left-column'),
        $leftWrapper = $this.find(
          '.slice-slider-left-column .slice-slider-wrapper'
        ),
      ...
      $this.find('.slice-slider-wrapper, .slice-align-animation').css({
        ...
        '-webkit-transition':
          'all ' + $settings.speed + 'ms ' + $settings.easing,
      })
        ...
        if (!mousewheelFinish && !$('body').hasClass('min-height')) {
          ...
          if (event.deltaY > 0) {
            ...
            $rightSlide
              ...
              .addClass('active')
            $this
              .find('.pagination .point.active:not(:first-child)')
              .removeClass('active')
              .prev()
              .addClass('active')
          } else {
            ...
            $rightSlide
              ...
              .addClass('active')
            $this
              .find('.pagination .point.active:not(:last-child)')
              .removeClass('active')
              .next()
              .addClass('active')
          }
