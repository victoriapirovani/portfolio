...
$(function () {
  ...
  _functions.pageCalculations = function () {
    ...
    var fullpageHeight =
      winH -
      $('header').not('.type-1').outerHeight() -
      $('footer').outerHeight()
    $('.full-screen-height').css({
      height: fullpageHeight < 500 ? 500 : fullpageHeight,
    })
  ...
  function setParams(swiper, dataValue, returnValue) {
    return swiper.is('[data-' + dataValue + ']')
      ? parseInt(swiper.data(dataValue), 10)
      : returnValue
  }
  ...
  $(window).on('load', function () {
    if ($('.sorting-container').length) {
      ...
      $('.sorting-menu a').on('click', function () {
        ...
        if ($(this).closest('.sorting-menu').find('.title').is(':visible'))
          $(this).closest('.sorting-menu').find('.toggle').slideUp()
  ...
  $('.toggle-mode.mode-2').on('click', function () {
    ...
    modeTimeout = setTimeout(function () {
      $('.product-shortcode-column').removeClass('change-mode')
    }, 100)
    return false
  })
