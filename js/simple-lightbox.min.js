...
!(function (a, b, c, d) {
  ...
  a.fn.simpleLightbox = function (d) {
    var s,
      ...
      w = function () {
        var b = (u(), 'pid=' + (G + 1)),
          d = t.href.split('#')[0] + '#' + b
        q
          ? history[r ? 'replaceState' : 'pushState']('', c.title, d)
          : r
          ? t.replace(d)
          : (t.hash = b),
          (r = !0)
      ...
      x = function () {
        q
          ? history.pushState('', c.title, t.pathname + t.search)
          : (t.hash = ''),
          clearTimeout(s)
      },
      y = function () {
        r ? (s = setTimeout(w, 800)) : w()
      },
      z = 'simplelb',
      A = a('<div>').addClass('sl-overlay'),
      B = a('<button>').addClass('sl-close').html(d.closeText),
      C = a('<div>').addClass('sl-spinner').html('<div></div>'),
      D = a('<div>')
      ...
      F = !1,
      G = 0,
      H = a('<div>').addClass('sl-caption pos-' + d.captionPosition),
      I = a('<div>').addClass('sl-image'),
      J = a('<div>').addClass('sl-wrapper').addClass(d.className),
      K = function (b) {
        return (
          !d.fileExt ||
      ...
      L = function () {
        d.close && B.appendTo(J),
          d.showCounter &&
            o.length > 1 &&
            (E.appendTo(J), E.find('.sl-total').text(o.length)),
          d.nav && D.appendTo(J),
          d.spinner && C.appendTo(J)
      },
      M = function (b) {
        b.trigger(a.Event('show.simplelightbox')),
          d.disableScroll && V('hide'),
          J.appendTo('body'),
          I.appendTo(J),
          d.overlay && A.appendTo(a('body')),
          (F = !0),
          (G = o.index(b)),
          ...
          C.show(),
          D.fadeIn('fast'),
          a('.sl-wrapper .sl-counter .sl-current').text(G + 1),
          E.fadeIn('fast'),
          N(),
          d.preloading && S(),
          setTimeout(function () {
        ...
        if (j.length) {
          ...
          ;(e.src = j.attr('src')),
            a(e).bind('error', function (b) {
              o.eq(G).trigger(a.Event('error.simplelightbox')),
                (F = !1),
                (l = !0),
                C.hide(),
            ...
            (e.onload = function () {
              ...
              var n =
                'self' == d.captionSelector
                  ? o.eq(G)
                  : o.eq(G).find(d.captionSelector)
              if ('data' == d.captionType) var q = n.data(d.captionsData)
              else if ('text' == d.captionType) var q = n.html()
              else var q = n.prop(d.captionsData)
              if (
                (d.loop ||
                  (0 == G && a('.sl-prev').hide(),
                  G >= o.length - 1 && a('.sl-next').hide(),
                  G > 0 && a('.sl-prev').show(),
                  G < o.length - 1 && a('.sl-next').show()),
                ...
                d.animationSlide &&
                  (p
                    ? (P(0, 100 * c + 'px'),
                      setTimeout(function () {
                        P(d.animationSpeed / 1e3, '0px'), 50
                      }))
                    : (r.left =
                        parseInt(a('.sl-image').css('left')) + 100 * c + 'px')),
                  a('.sl-image').animate(r, d.animationSpeed, function () {
                    ;(F = !1), O(q)
                  })
              } else (F = !1), O(q)
              d.additionalHtml &&
      ...
      Q = function () {
        a(b).on('resize.' + z, N),
          a(c).on('click.' + z + ' touchstart.' + z, '.sl-close', function (a) {
            a.preventDefault(), l && U()
          }),
          d.history &&
            setTimeout(function () {
              a(b).on('hashchange.' + z, function () {
                if (l && u() === v) return void U()
              })
            }, 40),
          D.on('click.' + z, 'button', function (b) {
            b.preventDefault(), (h = 0), T(a(this).hasClass('sl-next') ? 1 : -1)
          })
        var e = 0,
          f = 0,
          g = 0,
          ...
          k = !1,
          m = 0
        I.on('touchstart.' + z + ' mousedown.' + z, function (a) {
          return (
            !!k ||
            (p && (m = parseInt(I.css('left'))),
            (k = !0),
          ...
          .on(
            'touchmove.' + z + ' mousemove.' + z + ' pointermove MSPointerMove',
            function (a) {
              return (
                !k ||
                (a.preventDefault(),
          ...
          .on(
            ...
            function (a) {
              if (k) {
                ...
                d.loop ||
                  (0 == G && h < 0 && (b = !1),
                  G >= o.length - 1 && h > 0 && (b = !1)),
                  Math.abs(h) > d.swipeTolerance && b
                    ? T(h > 0 ? 1 : -1)
                    : d.animationSlide &&
                      (p
                        ? P(d.animationSpeed / 1e3, '0px')
                        : I.animate({ left: m + 'px' }, d.animationSpeed / 2)),
                  d.swipeClose &&
                    Math.abs(i) > 50 &&
      ...
      R = function () {
        D.off('click', 'button'),
          a(c).off('click.' + z, '.sl-close'),
          a(b).off('resize.' + z),
          a(b).off('hashchange.' + z)
      },
      S = function () {
        var b = G + 1 < 0 ? o.length - 1 : G + 1 >= o.length - 1 ? 0 : G + 1,
          c = G - 1 < 0 ? o.length - 1 : G - 1 >= o.length - 1 ? 0 : G - 1
        a('<img />')
          .attr('src', o.eq(b).attr('href'))
          .on('load', function () {
            m.indexOf(a(this).attr('src')) == -1 && m.push(a(this).attr('src')),
              o.eq(G).trigger(a.Event('nextImageLoaded.simplelightbox'))
          }),
          a('<img />')
            ...
            .on('load', function () {
              m.indexOf(a(this).attr('src')) == -1 &&
                ...
                o.eq(G).trigger(a.Event('prevImageLoaded.simplelightbox'))
            })
      },
      T = function (b) {
        o.eq(G)
        ...
        var c = G + b
        if (!(F || ((c < 0 || c >= o.length) && 0 == d.loop))) {
          ;(G = c < 0 ? o.length - 1 : c > o.length - 1 ? 0 : c),
            a('.sl-wrapper .sl-counter .sl-current').text(G + 1)
          var e = { opacity: 0 }
          d.animationSlide &&
            (p
              ? P(d.animationSpeed / 1e3, -100 * b - h + 'px')
              : (e.left =
            ...
            a('.sl-image').animate(e, d.animationSpeed, function () {
              setTimeout(function () {
                ...
                j.attr('src', c.attr('href')),
                  m.indexOf(c.attr('href')) == -1 && C.show(),
                  a('.sl-caption').remove(),
                  N(b),
                  d.preloading && S()
              }, 100)
      ...
      V = function (d) {
        if ('hide' == d) {
          ...
          if (c.body.clientWidth < e) {
            ...
            a(c.body)[0].removeChild(g),
              a('body').data('padding', h),
              i > 0 &&
                a('body')
                  .addClass('hidden-scroll')
                  .css({ 'padding-right': h + i })
          }
        } else
          a('body')
    ...
    return (
      L(),
      o.on('click.' + z, function (b) {
        if (K(this)) {
          if ((b.preventDefault(), F)) return !1
          M(a(this))
        }
      }),
      a(c).on('click.' + z + ' touchstart.' + z, function (b) {
        l &&
          d.docClose &&
      ...
      d.enableKeyboard &&
        a(c).on('keyup.' + z, function (a) {
          if ((a.preventDefault(), (h = 0), l)) {
            var b = a.keyCode
            27 == b && U(),
              (37 != b && 39 != a.keyCode) || T(39 == a.keyCode ? 1 : -1)
          }
        }),
